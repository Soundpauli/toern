...
void drawFilters(char *txt, int activeFilter) {
  ...
  if (SMP.currentChannel <10 && activeFilter>=7)   {
    drawLowResCircle(4,4,CRGB(100,50,0));
...
void displayCurrentView() {
  switch (fxType) {
    case 0:
      if (SMP.selectedParameter == 0)
        drawType(currentFilter, SMP.selectedParameter);
      else if (SMP.selectedParameter == 1)
        drawWaveforms(currentFilter, SMP.selectedParameter);
      else
...
void handleParameterEncoder() {
  if (currentMode->pos[0] != SMP.selectedParameter) {
    ...
    else if (SMP.selectedParameter == 1) drawWaveforms(currentFilter, SMP.selectedParameter);
    else
      drawADSR(currentFilter, SMP.selectedParameter);
  }
}
